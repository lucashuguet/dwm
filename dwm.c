...
struct Client {
  ...
  int isfixed, isfloating, canfocus, isurgent, neverfocus, oldstate,
      isfullscreen, cantfocus, isterminal, noswallow;
  pid_t pid;
  Client *next;
  Client *snext;
  Client *swallowing;
...
void focusstack(const Arg *arg) {
  ...
  if (arg->i > 0) {
    for (c = selmon->sel->next;
         c && (!ISVISIBLE(c) || !c->canfocus || c->cantfocus); c = c->next)
      ;
    if (!c)
    ...
    for (i = selmon->clients; i != selmon->sel; i = i->next)
      if (ISVISIBLE(i) && i->canfocus && !i->cantfocus)
        c = i;
    if (!c)
      for (; i; i = i->next)
        if (ISVISIBLE(i) && i->canfocus && !i->cantfocus)
...
void togglecanfocusfloating(const Arg *arg) {
  unsigned int n;
  Client *c, *cf = NULL;
